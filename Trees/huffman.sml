   ...
   let
      fun decode1([], currentBranch) = []
        | decode1(b::rest, currentBranch) =
           let 
	       val nextBranch = chooseBranch(b, currentBranch);
	   in
               case nextBranch of
		   Leaf(sym, w) =>  sym::decode1(rest, tree)
		 | Internal(left, right, syms, w) => decode1(rest, nextBranch)
           end;
   in
     decode1(bits, tree)
   end;
...
fun encodeSymbol(sym, tree) = 
   let 
      fun encodeSymbol1(sy, Leaf(st, w)) = if st = sy then SOME [] else NONE
        | encodeSymbol1(sy, Internal(left, right, st, w)) =
             case (encodeSymbol1(sy, left), encodeSymbol1(sy, right)) of
                (NONE, NONE) => NONE
              | (NONE, SOME bits) => SOME (One::bits)
              | (SOME bits, x) => SOME (Zero::bits);
    in
